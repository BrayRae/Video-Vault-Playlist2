...
export default function EditFolderScreen() {
  ...
  const { getFolder, createFolder, updateFolder } = usePlaylists();

  const editing = useMemo(
    () => (params.id ? getFolder(String(params.id)) : undefined),
    [params.id, getFolder],
  );

  const [name, setName] = useState(editing?.name ?? "");
  const [cover, setCover] = useState<string>(editing?.cover ?? "preset:2");
  ...
  const onSave = () => {
    ...
    if (editing) {
      updateFolder(editing.id, { name: trimmed, cover });
      router.back();
    } else {
      const created = createFolder({ name: trimmed, cover });
      router.dismiss();
      router.push({ pathname: "/folder/[id]", params: { id: created.id } });
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <PrimaryButton
          label={editing ? "Save Changes" : "Create Folder"}
          onPress={onSave}
          icon={editing ? "check" : "folder-plus"}
        />
      </View>
    </KeyboardAvoidingView>
  );
